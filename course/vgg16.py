...
# VGG 創作者首先將三個 (R, G, B) channel 各自的平均值減去，這樣每個聲道的數據平均為零。此外，他們的軟件預期 channel 是 B, G, R 順序，而 Python 默認使用 R, G, B。我們需要對數據進行預處理以做出這兩個更改，以便與 VGG 模型兼容    

# VGG 研究人員提供的每個 channel 的平均值
vgg_mean = np.array([123.68, 116.779, 103.939], dtype=np.float32)

def vgg_preprocess(x):
    """
        減去 RGB 平均值, 把 RGB 轉換成 BGR
...
class Vgg16():
    ...
    def get_classes(self):
        ...
        # get_file: keras 提供的方便函數，檔案不存在則會自動下載，預設儲存在 ~/.keras/datasets
        fpath = get_file(fname, CLASS_INDEX_PATH, cache_subdir='models', file_hash='c2c37ea517e94d9795004a39431a14cb')

        # 載入 imagenet_class_index.json 到 self.classes
    ...
    def create(self):
        ...
        # get_file: keras 提供的方便函數，檔案不存在則會自動下載，預設儲存在 ~/.keras/datasets
        model.load_weights(get_file(fname, WEIGHTS_PATH, cache_subdir='models'))
